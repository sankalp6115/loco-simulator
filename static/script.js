...
async function loadFrames() {
  try {
    const resp = await fetch('http://localhost:5000/frames');
    ...
    let loaded = 0;
    await Promise.all(frames.map(img => new Promise(res => {
      img.onload = () => {
        loaded++;
        progressFill.style.width = `${(loaded/totalFrames)*100}%`;
        progressText.textContent = `${Math.floor((loaded/totalFrames)*100)}%`;
        res();
      };
      img.onerror = () => res();
    })));
